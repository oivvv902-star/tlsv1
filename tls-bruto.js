...
const targetPort = parsedTarget.protocol === 'https:' ? 443 : 80;
    ...
    console.clear();
console.log('Script By : @nminh23'.rainbow);
console.log(`Target : ${parsedTarget.host}`.bgRed.white);
console.log(`Time : ${args.time.toString()} Second`.green);
console.log(`Thread(s) : ${args.threads.toString()}`.blue);
console.log(`Rate : ${args.Rate.toString()}`.yellow);
console.log(`ProxyFile: ${args.proxyFile} | Total : ${proxies.length}`.red);
  setTimeout(() => {
    process.exit(1);
  }, process.argv[3] * 1000);
